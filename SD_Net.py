...
class sd_net(nn.Module):
    ...
    def __init__(self, cfg, unet_config=dict()):
        ...
        self.f_net = filter_net()
        
        

    def forward(self, points, img, text,flag='train'):
        ...
        if flag!='train': 
            ...
            sd_outs = torch.reshape(sd_outs,[sd_outs.shape[0],-1,2048])
            sd_outs = torch.mean(sd_outs,dim=1).reshape(-1,2048)
